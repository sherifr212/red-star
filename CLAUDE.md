...
(`UnslothAgentFactory.Create`, `new ModelsClient(options)`) — tests substitute
`FakeChatClient`/`FakeModelsClient` there instead. This only covers the one-shot path and
...
`FakeAgentResponseExtractor` (`RedStar.UnitTest.Cli/Fakes/`) here instead of depending on real Unsloth
SSE JSON shapes.
